...
if (isset($_SESSION['residentID'])) {
    $userStmt = $conn->prepare("SELECT FirstName, LastName FROM resident WHERE ResidentID = ?");
    $userStmt->bind_param("s", $_SESSION['residentID']);
    $userStmt->execute();
    ...
    <div class="wrapper">
        ...
        <?php if (isset($_GET['applied'])): ?>
            <div class="apply-banner"
                style="background-color: #d4edda; color: #155724; padding: 15px; margin: 20px 0; text-align: center; border-radius: 5px;">
        ...
        <?php if (isset($_GET['already_applied'])): ?>
            <div class="apply-banner"
                style="background-color: #fff3cd; color: #856404; padding: 15px; margin: 20px 0; text-align: center; border-radius: 5px;">
                    ...
                    <div class="pet-card">
                        ...
                        <div class="pet-apply">
                            <?php if (!isset($_SESSION['residentID'])): ?>
                                <!-- Butang Log In -->
                                <button type="button" class="apply-btn" onclick="window.location.href='../User_Login.php';">Log In
                                    to Apply</button>
                            <?php else:
                                ...
                                if ($appStatus === 'Pending' || $appStatus === 'Submit'): ?>
                                    ...
                                    <button type="button" class="apply-btn" disabled
                                        style="background-color: #cccccc !important; color: #666666 !important; cursor: not-allowed;">Application
                                        Pending</button>
                                <?php elseif ($appStatus === 'Approved'): ?>
                                    <!-- Butang Dah Adopt -->
                                    <button type="button" class="apply-btn" disabled
                                        style="background-color: #cccccc !important; color: #666666 !important; cursor: not-allowed;">Already
                                        Adopted</button>
                                <?php else: ?>
                                    <!-- Form Submit - Pastikan butang di dalam tag form ini mempunyai class="apply-btn" -->
                                    <form method="POST" action="findapet.php"
                                        onsubmit="return confirm('Apply to adopt <?= htmlspecialchars($pet['PetName']) ?>?');"
