...
<head>
    ...
    <link rel="stylesheet" href="../css/petcommunityngo.css?v=2">
</head>
                    ...
                    <div class="community-card" id="board-<?= htmlspecialchars($boardID) ?>">
                        ...
                        <div class="community-image">
                            <?php if (!empty($post['Photo'])): ?>
                                <img src="<?= $photoFolder . htmlspecialchars($post['Photo']) ?>"
                        ...
                        <div class="community-content">
                            ...
                            <form class="edit-mode" data-board="<?= htmlspecialchars($boardID) ?>" method="POST"
                                action="petcommunity.php" style="display:none;">
                                <input type="hidden" name="action" value="update_post">
                                <input type="hidden" name="BoardID" value="<?= htmlspecialchars($boardID) ?>">
                                <input type="text" name="Title" class="edit-title-input"
                                    value="<?= htmlspecialchars($post['Title']) ?>" required>
                                <textarea name="Content" class="edit-content-input"
                                    required><?= htmlspecialchars($post['Content']) ?></textarea>
                                <div class="edit-actions">
                                    <button type="submit" class="save-btn">Save</button>
                                    <button type="button" class="cancel-btn"
                                        onclick="toggleEdit('<?= htmlspecialchars($boardID) ?>')">Cancel</button>
                                </div>
                            </form>
                            ...
                            <div class="panel" id="panel-<?= htmlspecialchars($boardID) ?>">
                                            ...
                                            <div class="comment-item <?= $comment['ReplyID'] ? 'comment-reply' : '' ?>"
                                                ...
                                                <div class="comment-view" data-comment="<?= htmlspecialchars($comment['CommentID']) ?>">
                                                    <?php if ($comment['ReplyID']): ?>
                                                        <span class="reply-badge">↳ replying to
                                                            <?= htmlspecialchars($comment['ReplyToName'] ?? $comment['ReplyID']) ?></span>
                                                    <?php endif; ?>
                                                    <div>
                                                    ...
                                                    <div class="comment-footer">
                                                        <span
                                                            class="comment-date"><?= date('d M Y, g:i A', strtotime($comment['Date'])) ?></span>
                                                        <div class="comment-actions">
                                                            ...
                                                            <form method="POST" action="petcommunity.php"
                                                                onsubmit="return confirm('Delete this comment?');"
                                                                style="display:inline;">
                                                                <input type="hidden" name="action" value="delete_comment">
                                                                <input type="hidden" name="CommentID"
                                                                    value="<?= htmlspecialchars($comment['CommentID']) ?>">
                                ...
                                <form class="ngo-reply-form" id="ngo-reply-form-<?= htmlspecialchars($boardID) ?>" method="POST"
                                    action="petcommunity.php" style="display:none;">
                                    <input type="hidden" name="action" value="ngo_reply">
                                    <input type="hidden" name="BoardID" value="<?= htmlspecialchars($boardID) ?>">
                                    <input type="hidden" name="ReplyID" id="ngo-reply-id-<?= htmlspecialchars($boardID) ?>"
                                        value="">
                                    <div class="comment-input-row">
                                        <span class="reply-to-label"
                                            id="ngo-reply-label-<?= htmlspecialchars($boardID) ?>"></span>
                        ...
                        <div class="community-actions">
                            <button type="button" onclick="toggleComments('<?= htmlspecialchars($boardID) ?>')"
                                title="View comments">💬</button>
                            <button type="button" onclick="toggleEdit('<?= htmlspecialchars($boardID) ?>')"
                                title="Edit">✏️</button>
                            <form method="POST" action="petcommunity.php"
                                onsubmit="return confirm('Delete this post? This cannot be undone.');" style="display:inline;">
